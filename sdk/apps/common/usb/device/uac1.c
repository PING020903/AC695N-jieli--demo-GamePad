...
static const u8 sConfigDescriptor_interface_1[0x40] =
{
    ...
    0x07, 0x05, 0x05, 0x03, 0x20, 0x00, 0x10,
};
...
u32 uac_audio_desc_config(const usb_dev usb_id, u8 *ptr, u32 *cur_itf_num)
{
    ...
    memcpy(tptr, (u8 *)uac_ac_standard_interface_desc, sizeof(uac_ac_standard_interface_desc));//添加uac_ac_standard_interface_desc
    tptr[2] = *cur_itf_num;
    tptr += sizeof(uac_ac_standard_interface_desc);//0x09, 偏移9bit

    memcpy(tptr, (u8 *)uac_audio_ac_interface, sizeof(uac_audio_ac_interface));//添加uac_audio_ac_interface
#if SPK_CHANNEL == 1
    tptr[5]--;
#endif
#if MIC_CHANNEL == 2
