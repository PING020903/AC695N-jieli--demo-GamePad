...
ifeq ($(SoC),br30)
ifeq ($(APP_CASE),hid)
ifneq (,$(wildcard $(ROOT)/cpu/$(CPU_DIR)/tools/download_v2.c))
	@$(CC) -MM $(SYS_INCLUDES) $(includes) -D__LD__ $(CC_DEFINE) $(ROOT)/cpu/$(CPU_DIR)/tools/download_v2.c -o $(ROOT)/cpu/$(CPU_DIR)/tools/download_v2.d || true 
	$(V) $(CC) $(SYS_INCLUDES) $(includes) -E -D__LD__ $(CC_DEFINE) -P $(ROOT)/cpu/$(CPU_DIR)/tools/download_v2.c -o $(POST_BUILD_SCRIPT) || true 
endif
ifneq (,$(wildcard $(ROOT)/cpu/$(CPU)/tools/isd_config_rule.c))
	$(CC) -MM $(SYS_INCLUDES) $(includes) -D__LD__ $(CC_DEFINE) $(ROOT)/cpu/$(CPU)/tools/isd_config_rule.c -o $(ROOT)/cpu/$(CPU)/tools/isd_config_rule.d || true 
	$(CC) $(SYS_INCLUDES) $(includes) -E -D__LD__ $(CC_DEFINE) -P $(ROOT)/cpu/$(CPU)/tools/isd_config_rule.c -o $(ROOT)/cpu/$(CPU)/tools/isd_config.ini || true 
endif 
else  #br30 other case
