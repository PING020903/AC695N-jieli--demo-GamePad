...
{
    printf("\n======== PWM LED CONFIG ======\n");
    printf("P3_CLK_CON0 	= 0x%x\n", p33_rx_1byte(P3_CLK_CON0));
    printf("PWM_CON0 	= 0x%x\n", JL_PLED->CON0);
    printf("PWM_CON1 	= 0x%x\n", JL_PLED->CON1);
    printf("PWM_CON2 	= 0x%x\n", JL_PLED->CON2);
    printf("PWM_CON3 	= 0x%x\n", JL_PLED->CON3);
    printf("PRD_DIVL     = 0x%x\n", JL_PLED->PRD_DIVL);

    printf("BRI_PRDL      = 0x%x\n", JL_PLED->BRI_PRDL);
    printf("BRI_PRDH      = 0x%x\n", JL_PLED->BRI_PRDH);

    printf("BRI_DUTY0L    = 0x%x\n", JL_PLED->BRI_DUTY0L);
    printf("BRI_DUTY0H    = 0x%x\n", JL_PLED->BRI_DUTY0H);

    printf("BRI_DUTY1L    = 0x%x\n", JL_PLED->BRI_DUTY1L);
    printf("BRI_DUTY1H    = 0x%x\n", JL_PLED->BRI_DUTY1H);

    printf("PWM1_DUTY0    = 0x%x\n", JL_PLED->DUTY0);
    printf("PWM1_DUTY1    = 0x%x\n", JL_PLED->DUTY1);
    printf("PWM1_DUTY2    = 0x%x\n", JL_PLED->DUTY2);
    printf("PWM1_DUTY3    = 0x%x\n", JL_PLED->DUTY3);
}
                                     ...
                                     u32 led0_light_delay_time, u32 led1_light_delay_time, u32 led_blink_delay_time)
{
    ...
    u16 pwm1_div = 0;
    u16 Tpwm1 = 0;
    pwm1_div = led0_bri_duty > led1_bri_duty ? led0_bri_duty : led1_bri_duty;//在循环中，这个语句配合下面的分支选择，即可选择两个中的一个led
 ...
 * @return void
 */
void _my_pwm_led_breathe_display(u8 led_index, u16 breathe_time, u16 led0_bright, u16 led1_bright,
                                     u32 led0_light_delay_time, u32 led1_light_delay_time, u32 led_blink_delay_time)
{
    u16 led1_bri_duty = led1_bright;
    u16 pwm1_div = 0;
    u16 Tpwm1 = 0;
    pwm1_div = led1_bri_duty;
    ...
    //step3: bright亮度
    _led_pwm_bright_set(500, led0_bri_duty, 0);
    ...
    _led_pwm1_duty_set(pwm1_duty3, pwm1_duty1, pwm1_duty0, pwm1_duty2, 1);
...
#endif
#if 1
void my_pwm_led_on_display(u8 led_index, u16 led0_bright, u16 led1_bright)
{
    //step1: pwm0 clock
    if (__this->clock == PWM_LED_CLK_RC32K) {
        _led_pwm0_clk_set(PWM0_CLK_32K);
...
void my_PWM_bright_set(u16 led1_duty)
{
    _led_pwm_bright_set(200, 0, ((led1_duty * 200) / 500));
    _led_pwm1_duty_set(40, 2, 0, 0, 0);
    _led_pwm_module_enable(1, 0);
}
#endif
